...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .setDescription("Show a leaderboard of the weekly most active users in the server"),
    /**
     *
     *
    ...
    async execute(interaction, client, config) {
        ...
        data.sort((a, b) => {
            if (a.data.timeBasedStats.weeklyMessages === b.data.timeBasedStats.weeklyMessages) {
                return a.key.userId - b.key.userId;
            } else {
                return b.data.timeBasedStats.weeklyMessages - a.data.timeBasedStats.weeklyMessages;
            }
        });
        const currentRank = data.findIndex((msg) => msg.key.userId === user.id) + 1;
        const top = data.slice(0, 10);
        const embed = new EmbedBuilder()
            .setTitle(`**Top 10 weekly active users**`)
            .setColor("Purple")
            .setTimestamp()
            .setFooter({
                text: `You are rank ${currentRank}. with ${new Big(
                    userData.data.timeBasedStats.weeklyMessages
                )
                    .toString()
        ...
        for (let i = 0; i < top.length; i++) {
            ...
            if (!user) return;
            const messagecount = new Big(top[i].data.timeBasedStats.weeklyMessages)
                .toString()
                .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
